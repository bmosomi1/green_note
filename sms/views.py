    ...
    if request.method == 'POST':
        ...
        if type(search_key)==str:
            ...
            try:
                ...
                bill_sent = WaterBillSent.objects.raw('SELECT * FROM sms_waterbillsent where account_number= %s or dest_msisdn=%s', [search_key, search_key])
...
def water_apps(request):
    ...
    if customer is None:
        ...
        for month in months:
            # print(week)
            monthly_units = int(WaterMeterReadings.objects.filter(read_date__month=this_month).aggregate(total=Sum('units_consumed'))['total'] or 0)
...
def meter_readings_report(request):
    ...
    if request.method == 'POST':
        ...
        if report_value=='1':
            ...
            #summary_sheet.append(('GNOTE WATER CLIENTS'))
            summary_sheet.append(('GWATER CLIENTS','NAMES'))
